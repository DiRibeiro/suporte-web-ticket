<template>
  <section>
    <header class="list-header">
      <h2>Técnicos</h2>
      <router-link class="btn" to="/admin/tecnicos/novo">+ Novo Técnico</router-link>
    </header>

    <div v-if="carregando">Carregando...</div>

    <div v-else>
      <table class="table" v-if="tecnicos.length">
        <thead>
          <tr>
            <th>ID</th>
            <th>Nome</th>
            <th>E-mail</th>
            <th>Especialidades</th>
            <th>Ações</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="t in tecnicos" :key="t.id">
            <td>{{ t.id }}</td>
            <td>{{ t.nome }}</td>
            <td>{{ t.email }}</td>
            <td>{{ (t.especialidades || []).join(', ') }}</td>
            <td>
              <router-link
                class="btn btn-small"
                :to="`/admin/tecnicos/${t.id}/editar`"
              >
                Editar
              </router-link>
              <button class="btn btn-small btn-danger" @click="confirmarDelete(t.id)">
                Excluir
              </button>
            </td>
          </tr>
        </tbody>
      </table>

      <p v-else>Nenhum técnico encontrado.</p>
    </div>
  </section>
</template>

<script setup>
import { onMounted, ref } from 'vue'
import { listarTecnicos, deletarTecnico } from '../api/tecnicos'

const tecnicos = ref([])
const carregando = ref(false)

async function carregar() {
  try {
    carregando.value = true
    const { data } = await listarTecnicos({ page: 0, size: 50 })
    tecnicos.value = data.content || []
  } catch (e) {
    console.error(e)
    alert('Erro ao carregar técnicos')
  } finally {
    carregando.value = false
  }
}

async function confirmarDelete(id) {
  if (!confirm('Tem certeza que deseja excluir este técnico?')) return
  try {
    await deletarTecnico(id)
    await carregar()
  } catch (e) {
    console.error(e)
    alert('Erro ao excluir técnico')
  }
}

onMounted(carregar)
</script>

<style scoped>
.list-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 1rem;
}

.btn {
  background-color: #1e40af;
  color: #fff;
  border: none;
  padding: 0.4rem 0.8rem;
  border-radius: 4px;
  cursor: pointer;
  text-decoration: none;
  font-size: 0.9rem;
}

.btn:hover {
  opacity: 0.9;
}

.btn-small {
  padding: 0.2rem 0.5rem;
  margin-right: 0.3rem;
}

.btn-danger {
  background-color: #b91c1c;
}

.table {
  width: 100%;
  border-collapse: collapse;
}

.table th,
.table td {
  border: 1px solid #ddd;
  padding: 0.4rem 0.6rem;
  font-size: 0.9rem;
}

.table th {
  background-color: #f3f4f6;
  text-align: left;
}
</style>
